BINARY_DIR := bin
IMAGE_BASE := ghcr.io/yourorg/socialpublish
GO := go
...
lint:
	if ! command -v $(GOLANGCI_LINT) >/dev/null 2>&1; then $(GO) install github.com/golangci/golangci-lint/cmd/$(GOLANGCI_LINT)@v1.57.0; fi
	$(GOLANGCI_LINT) run ./...
